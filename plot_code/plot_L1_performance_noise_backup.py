...
#         target_train_group = (293.15, len(substance_list), tuple(substance_list), 7, (0, 1, 2, 3, 4, 5, 6), train_noise, 1)
...
#         plt.plot(index, filtered_train_df[f'Substance {substance} skewnorm {confidence_perc*100}% interval range'], label=f'Original')
#         # plt.plot(index, filtered_train_df_pca[f'Substance {substance} skewnorm {confidence_perc*100}% interval range'], label=f'PCA')

#         # Set plot labels and title
#         plt.xlabel('Max Test Noise (Absolute value added after normalization)')
#         plt.ylabel(f'skewnorm {confidence_perc*100}% interval range')
#         plt.title(f'Substance {substance} skewnorm\n{confidence_perc*100}% interval range (Performance) vs Noise Level')
...
#         target_group_mtx_mul = (293.15, len(substance_list), tuple(substance_list), 7, (0, 1, 2, 3, 4, 5, 6))
...
for train_noise in train_noise_list:
    ...
    index = range(len(filtered_train_df['Test Noise Max Percentage']))
    plt.plot(index, filtered_train_df[f'MSELoss'], label=f'Train Noise={train_noise}')
...
#     plt.figure()
    ...
    #     # Create the plot
    #     print(data[f'L1Loss'])
    #     plt.plot(index, data[f'L1Loss'], label=f'Test Noise Max Percentage: {test_noise_max_percentage}')
